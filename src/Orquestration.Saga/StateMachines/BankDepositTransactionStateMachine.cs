...
namespace Orquestration.Saga.StateMachines
{
    public sealed class BankDepositTransactionStateMachine : AcessoStateMachine<BankDepositTransactionSaga>
    {
        ...
        public BankDepositTransactionStateMachine(ILogger<BankDepositTransactionStateMachine> logger)
        {
            ...
            During(Processing, AwaitingFraudAnalyze,
                When(BankDepositTransactionWasApproved)
                    ...
                    .ThenAsync(context => SendCommandAsync<IReserveCustomerBankDepositLimitAmount>(new ReserveCustomerBankDepositLimitAmount { CorrelationId = context.Data.CorrelationId }, new Uri("rabbitmq://localhost/core-limit-queue"), context))
                    //.Finalize());
            ...
            ////        //.ThenAsync(context => TODO: Send request to move amount to regular account)
                    .ThenAsync(context => SendCommandAsync<IMoveReservedLimitAmountToRegularAccount>(new MoveReservedLimitAmountToRegularAccount { CorrelationId = context.Data.CorrelationId }, new Uri("rabbitmq://localhost/core-account-queue"), context))
